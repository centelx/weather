...
def train_model(X, y_residual, y_true_absolute, baseline_absolute, model_name, output_dir, args=None):
    X = X.sort_index()
    ...
    if args and getattr(args, 'deep', False):
        xgb_param_dist = {
            'regressor__max_depth': [2, 3, 4, 5, 6],
            'regressor__learning_rate': [0.005, 0.01, 0.05, 0.1, 0.2],
    ...
    xgb_search = RandomizedSearchCV(
        ...
        n_iter=n_iters, cv=tscv, scoring='neg_mean_absolute_error',
...
def run_training(city, project_root, args):
    ...
    print(f"Dane od {df['Date'].min().date()} do {df['Date'].max().date()} wczytane pomyslnie.")
    
    models_dir = os.path.join(project_root, 'models_temp')
    ...
    train_model(
        ...
        model_name=f"{city}_model_max_aws", 
    ...
    train_model(
        ...
        model_name=f"{city}_model_min_aws", 
        output_dir=models_dir,
        args=args
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--deep", action="store_true", help="Gleboka optymalizacja 500 iteracji")
    args = parser.parse_args()
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(current_dir)
    
    print(f"Rozpoczęcie potoku dla miasta: {args.city.upper()} (Model: Połączony GFS i HRRR)")
    run_training(args.city, project_root, args)

...
class Backtester:
                ...
                if self.min_price <= p_market_yes <= self.max_price:
                    ...
                    if expected_roi_yes > self.edge_threshold and self.side in ['ALL', 'YES']:
                        ...
                        self.history.append({
                            ...
                            'profit': profit,
                            'is_win': is_hit,
                            'actual_temp': actual_max if trade['type'] == 'MAX' else actual_min
                        })
    ...
    def generate_report(self, base_path):
        ...
        # Zapis historii zakładów do pliku TXT
        out_txt = os.path.join(base_path, 'trades_history.txt')
        with open(out_txt, 'w', encoding='utf-8') as f:
            f.write("--- HISTORIA ZAŁOŻONYCH ZAKŁADÓW ---\n\n")
            for trade in self.history:
                strona = "TAK" if "_YES" in trade['type'] else "NIE"
                wynik = "WYGRANY" if trade['is_win'] else "PRZEGRANY"
                f.write(f"Data: {trade['date']} | Typ: {trade['type'].split('_')[0]} | Koszyk: {trade['bucket']} | Faktyczna Temp: {trade['actual_temp']} | Strona: {strona} | Prawd. Rynku: {trade['p_market']*100:.1f}% | Nasz Edge: {trade['exp_roi']*100:.1f}% | Wynik: {wynik} | Zysk: ${trade['profit']:.2f}\n")
        print(f"Zapisano historię zakładów do {out_txt}")

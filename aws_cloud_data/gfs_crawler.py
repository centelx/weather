...
class GFSCrawler:
    def __init__(self, city_name, lat, lon, timezone_str="America/New_York"):
        """
        Inicjalizacja crawlara GFS dla konkretnego miasta.
        """
        self.city_name = city_name
        self.target_lat = lat
        
        # GFS używa długości od 0 do 360
    ...
    def _process_single_hour(self, fcst, run_date_str):
        ...
        idx_url = f"{self.bucket_base}/gfs.{run_date_str}/12/atmos/gfs.t12z.pgrb2.0p25.f{fcst:03d}.idx"
        grib_url = f"{self.bucket_base}/gfs.{run_date_str}/12/atmos/gfs.t12z.pgrb2.0p25.f{fcst:03d}"
        ...
        for v_key, (s_byte, e_byte) in ranges.items():
            ...
            try:
                ...
                with self._cfgrib_lock:
                    ...
                    # W GFS var_names są nieco inne w xarray, ale zazwyczaj podobne do GFS
                    # Ponieważ użyliśmy PRATE zamiast APCP, xarray nazwie to 'prate'
                    variables = {
                    ...
                    x_var = variables.get(v_key, list(ds.data_vars)[0])
                    if x_var not in ds:
                    ...
                    if lats.ndim == 1 and lons.ndim == 1:
                        ...
                        lat_s = max(0, lat_idx - 1)
                        lat_e = min(len(lats), lat_idx + 2)
                        lon_s = max(0, lon_idx - 1)
                        lon_e = min(len(lons), lon_idx + 2)
        ...
        # Przeliczenie opadów PRATE z kg/m^2/s na mm/h we wszystkich statystykach
        for stat in ["mean", "max", "min", "std"]:
            k = f"APCP_{stat}"
            if k in hour_record:
                hour_record[k] = hour_record[k] * 3600.0
    ...
    def fetch_forecast_for_target_day(self, target_date_str):
        ...
        print(f"Fetching GFS for {target_date_str} using run {run_date_str} 12Z. Forecast range: {fcst_start}h to {fcst_end}h")
        ...
        if "TMP_mean" in df.columns:
            ...
            result[f'temperature_2m_max_previous_{conf}_gfs'] = df["TMP_max_C"].max()
            result[f'temperature_2m_min_previous_{conf}_gfs'] = df["TMP_min_C"].min()
            result[f'temperature_2m_mean_previous_{conf}_gfs'] = df["TMP_mean_C"].mean()
            result[f'temperature_2m_std_previous_{conf}_gfs'] = df["TMP_std"].mean()
            
        if "RH_mean" in df.columns:
            result[f'relative_humidity_2m_mean_previous_{conf}_gfs'] = df["RH_mean"].mean()
            result[f'relative_humidity_2m_std_previous_{conf}_gfs'] = df["RH_std"].mean()
            
        if "TCDC_mean" in df.columns:
            result[f'cloud_cover_mean_previous_{conf}_gfs'] = df["TCDC_mean"].mean()
            result[f'cloud_cover_std_previous_{conf}_gfs'] = df["TCDC_std"].mean()
            
        if "APCP_mean" in df.columns:
            result[f'precipitation_sum_previous_{conf}_gfs'] = df["APCP_mean"].sum()
            result[f'precipitation_max_previous_{conf}_gfs'] = df["APCP_max"].max()
            result[f'precipitation_std_previous_{conf}_gfs'] = df["APCP_std"].mean()
            
        if "UGRD_mean" in df.columns and "VGRD_mean" in df.columns:
            df["wind_speed_mean"] = np.sqrt(df["UGRD_mean"]**2 + df["VGRD_mean"]**2) * 3.6
            result[f'wind_speed_10m_max_previous_{conf}_gfs'] = df["wind_speed_mean"].max()
            result[f'wind_speed_10m_mean_previous_{conf}_gfs'] = df["wind_speed_mean"].mean()

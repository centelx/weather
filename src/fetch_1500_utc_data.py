...
def fetch_real_polymarket_data(base_path, city):
    real_prices_file = os.path.join(base_path, 'data', 'real_prices', f'real_polymarket_prices_{city}_with_no.csv')
    if not os.path.exists(real_prices_file):
        print(f"Brak pliku {real_prices_file}.")
        return
        
    df_real = pd.read_csv(real_prices_file)
    unique_dates = df_real['target_date'].unique()
        ...
        for slug, temp_type in slugs_to_try:
            ...
            if res.status_code == 200 and len(res.json()) > 0:
                ...
                for m in markets:
                    ...
                    for horizon in [1]:
                        cutoff_dt = datetime.strptime(f"{date_str} 15:00:00", "%Y-%m-%d %H:%M:%S").replace(tzinfo=timezone.utc) - pd.Timedelta(days=1)
    ...
    out_csv = os.path.join(base_path, '1_day_1200_polymarket_data', f'real_polymarket_prices_{city}_with_no.csv')

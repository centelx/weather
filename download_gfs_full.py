...
def main():
    ...
    crawler = GFSCrawler(city, lat, lon, timezone)
    dates_to_fetch = pd.date_range(start=start_date, end=end_date)
    
    out_file = os.path.join("aws_datasets", f"features_{city}_gfs_full.csv")
    ...
    for t in dates_to_fetch:
        ...
        if daily_features:
            ...
            print(f"[BRAK DANYCH] Nie znaleziono pełnych danych GFS dla {date_str} na S3.")

    # Sort the final CSV chronologically so 2021 is at the top
    if os.path.exists(out_file):

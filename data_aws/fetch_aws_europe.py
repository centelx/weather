...
from icon_eu_crawler import ICONEUCrawler
...
def main():
    parser = argparse.ArgumentParser(description="Pobieranie GFS i ICON-EU dla Europy")
    ...
    if config.get('secondary_model') != "ICON-EU":
        print(f"Pominięto {city}: ten skrypt służy tylko dla miast z secondary_model=ICON-EU.")
        return
    ...
    # 2. ICON-EU
    icon_crawler = ICONEUCrawler(city, lat, lon, tz)
    fetch_for_model(city, icon_crawler, config, "icon-eu")

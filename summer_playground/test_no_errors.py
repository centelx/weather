...
if __name__ == "__main__":
    ...
    out_file = os.path.join(current_dir, "last_errors_none.txt")
    with open(out_file, "w") as f:
        f.write("WYNIKI OPTYMALIZACJI: CALKOWITY BRAK CECH BLEDOW GFS/HRRR\n")
        f.write("="*60 + "\n\n")
    ...
    opt_m_max = get_base_optimal_features(rankings["miami_max"][:24])
    y_tr = df_m[tr_m]["Max_Temp"] - df_m[tr_m]["baseline_max"]
    y_val = df_m[val_m]["Max_Temp"]
    base_val = df_m[val_m]["baseline_max"]
    
    mae_m_max = run_training(df_m[tr_m][opt_m_max], y_tr, df_m[val_m][opt_m_max], y_val, base_val, "svr", "standard")
    ...
    mae_m_min = run_training(df_m[tr_m][base_all_m], y_tr, df_m[val_m][base_all_m], y_val, base_val, "xgb", "robust")
    ...
    opt_la_max = get_base_optimal_features(rankings["losangeles_max"][:12])
    y_tr = df_la[tr_la]["Max_Temp"] - df_la[tr_la]["baseline_max"]
    y_val = df_la[val_la]["Max_Temp"]
    base_val = df_la[val_la]["baseline_max"]
    
    mae_la_max = run_training(df_la[tr_la][opt_la_max], y_tr, df_la[val_la][opt_la_max], y_val, base_val, "svr", "robust")
    ...
    mae_la_min = run_training(df_la[tr_la][base_all_la], y_tr, df_la[val_la][base_all_la], y_val, base_val, "xgb", "robust")
    ...
    print("Testy bez bledu zakonczone. Wyniki w last_errors_none.txt")

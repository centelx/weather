...
def get_data(city):
    ...
    # Lags computed BEFORE dropping non-summer months!
    ...
    # Drop rows with NaN in features (e.g., due to lags at the start of the entire dataset)
    df = df.dropna(subset=features + ['Date']).reset_index(drop=True)
    
    # NOW filter specifically to only July-Oct
    df = df[df['Date'].dt.month.isin([7, 8, 9, 10])].reset_index(drop=True)
...
def train_and_evaluate(df, features, target_col, baseline_col, model_name, algo_type="svr"):
    # Podejscie 4 (Lipiec-Pazdziernik):
    # Zbiór testowy: TYLKO 2025-07-01 do 2025-10-31
    
    val_mask = (df['Date'] >= pd.to_datetime('2025-07-01')) & (df['Date'] <= pd.to_datetime('2025-10-31'))
    train_mask = ~val_mask
    ...
    print(f"[{model_name}] Letni Test MAE: {mae:.4f}, RMSE: {rmse:.4f} (Train: {len(X_train)}, Val: {len(X_val)})")
...
if __name__ == "__main__":
    ...
    # 3. LA MAX (SVR N=12)
    df_la, all_features_la = get_data("losangeles")
    features_max_la = rankings["losangeles_max"][:12]
    r3 = train_and_evaluate(df_la, features_max_la, "Max_Temp", "baseline_max", "losangeles_model_max_summer_pure", algo_type="svr")
    results.append(r3)

    # 4. LA MIN (XGBoost N=ALL)
    r4 = train_and_evaluate(df_la, all_features_la, "Min_Temp", "baseline_min", "losangeles_model_min_summer_pure", algo_type="xgb")
    results.append(r4)
    
    print("Wytrenowano i zapisano wyspecjalizowane LA letnie modele!")
    
    summary_path = os.path.join(current_dir, "summer_4.txt")
    with open(summary_path, 'w') as f:
        f.write("WYNIKI - PODEJŚCIE 4 (LA Lipiec-Październik)\n")
        f.write("-----------------------------------------------------------\n")
        f.write("Zbiór testowy: TYLKO 01.07.2025 - 31.10.2025\n")
        f.write("Zbiór treningowy: Lipiec-Paź 2021, 2022, 2023, 2024, oraz Lipiec 2026\n\n")
        f.write("LOS ANGELES:\n")
        f.write(f"- MAX (SVR N=12): {results[0]}\n")

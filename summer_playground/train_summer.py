...
def train_and_evaluate(df, features, target_col, baseline_col, model_name, algo_type="svr"):
    # Podejscie 2: Zlepek lata 2025 i 2026 jako test, wszystko inne jako trening
    val_mask_2025 = (df['Date'] >= pd.to_datetime('2025-06-01')) & (df['Date'] <= pd.to_datetime('2025-09-30'))
    val_mask_2026 = (df['Date'] >= pd.to_datetime('2026-06-01'))
    val_mask = val_mask_2025 | val_mask_2026
    train_mask = ~val_mask
...
if __name__ == "__main__":
    ...
    # 1. Miami MAX (SVR N=24)
    df_m, all_features_m = get_data("miami")
    features_max_m = rankings["miami_max"][:24]
    train_and_evaluate(df_m, features_max_m, "Max_Temp", "baseline_max", "miami_model_max_summer", algo_type="svr")

    # 2. Miami MIN (SVR N=12)
    features_min_m = rankings["miami_min"][:12]
    train_and_evaluate(df_m, features_min_m, "Min_Temp", "baseline_min", "miami_model_min_summer", algo_type="svr")

    # 3. LA MAX (SVR N=12)
    df_la, all_features_la = get_data("losangeles")
    features_max_la = rankings["losangeles_max"][:12]
    train_and_evaluate(df_la, features_max_la, "Max_Temp", "baseline_max", "losangeles_model_max_summer", algo_type="svr")

    # 4. LA MIN (XGBoost N=ALL)
    train_and_evaluate(df_la, all_features_la, "Min_Temp", "baseline_min", "losangeles_model_min_summer", algo_type="xgb")
    
    print("Wytrenowano i zapisano wyspecjalizowane letnie modele!")
